...
from pyspark.sql.functions import desc, row_number, dense_rank, col, lit, explode, input_file_name, split
...
dbutils.widgets.text("SystemLoadID"       , "1052022061701"                                   , "SystemLoadID")
dbutils.widgets.text("SourcePath"         , "/silver/underwriting/Internal/Eclipse/Staging"   , "SourcePath")
dbutils.widgets.text("TargetPath"         , "/silver/underwriting/Internal/Eclipse/DeltaLake" , "TargetPath")
dbutils.widgets.text("TargetDatabaseName" , "EclipseSilver"                                   , "TargetDatabaseName")
...
dbutils.widgets.text("WatermarkValue"     , "1042022061701"                                   , "WatermarkValue")
...
arr_sourceFilePath     = [x.path for x in dbutils.fs.ls(str_sourceFilePath) if int(x.name.split("=")[1].split("/")[0]) > int_watermarkValue]
...
try:
    df = (spark
          ...
          .parquet(*arr_sourceFilePath) 
          .withColumn("SystemLoadID", split(split(input_file_name(),"=")[1],"/")[0])
         )
except: 
    ...
    df = (spark
          .read
          .option("mergeSchema","true") 
          .json(*arr_sourceFilePath) 
          .withColumn("data", explode("data"))
          .select("data.*", 
                  split(split(input_file_name(),"=")[1],"/")[0].alias("SystemLoadID")
                 )
         )
