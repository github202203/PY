...
# MAGIC %sql
# MAGIC DESC vw_reconciliationOutput

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM vw_reconciliationOutput
...
# MAGIC %sql
# MAGIC DESC reconciliation.reconciliation_output
...
try:          
  spark.sql(f"""
    ...
    ON t.DestinationName         = s.DestinationName
      ...
      AND IFNULL(t.ControlID, 0) = IFNULL(s.ControlID, 0)      
      AND t.SourceSystemName     = s.SourceSystemName
      AND t.DestinationSubjectID = s.DestinationSubjectID
      AND t.DestinationEntityID  = s.DestinationEntityID 
      AND t.ReconciliationType   = s.ReconciliationType      
